...
public class TRAODELK1993Copy extends Applet {

    public static double TH_DILTA = 55;
    public final static double D = 900.0D;
    public final static double p = 0.95D;
    ...
    public void init() {
        ...
        List<TFragment> fragments = new ArrayList<TFragment>();
        {
            ...
            System.out.println("轨迹划分数：" + fragments.size());
    ...
    public void drawTFragment(Graphics g, TFragment tf, Color color) {
        g.setColor(color);
    ...
    public void drawTPoint(Graphics g, TPoint tp, Color color) {
        g.setColor(color);
        //g.drawLine(5, 4, 100, 50);
    ...
    public void paint(Graphics g) {
        ...
        Trajectory trajectory = input_trajs.get(1);
        /*for (int i = 0; i < 90; i++) {
            trajectory.points.add(input_trajs.get(1).points.get(i));
        }*/
        System.out.println("size:" + trajectory.points.size());
        for (int i = 1; i < trajectory.points.size(); i++) {
            TFragment tf = new TFragment(trajectory.points.get(i - 1), trajectory.points.get(i));
            drawTFragment(g, tf, Color.GREEN);
        }
        //drawTPoint(g, trajectory.points.get(0), Color.BLUE);
        //drawTPoint(g, trajectory.points.get(trajectory.points.size() - 1), Color.BLUE);

        Partition partition = new Partition(trajectory);
        trajectory = partition.getTrajectoryPartition();

        /*TrajectorySimplify simplify = new TrajectorySimplify(trajectory);
        trajectory = simplify.getSimplifiedTrajectoty();*/
        System.out.println("SimplyfiSize:" + trajectory.points.size());
        for (int i = 1; i < trajectory.points.size(); i++) {
            TFragment tf = new TFragment(trajectory.points.get(i - 1), trajectory.points.get(i));
