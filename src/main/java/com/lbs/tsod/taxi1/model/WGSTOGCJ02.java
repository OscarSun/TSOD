...
public class WGSTOGCJ02 {
    ...
    public Coordinate transform(double lon, double lat) {
        if (outofChina(lat, lon)) return new Coordinate(lon, lat);
    ...
    // gcj02->84
    public Coordinate gcj2wgs(double lon, double lat) {
        double lontitude = lon - (transform(lon, lat).x - lon);
        double latitude = lat - (transform(lon, lat).y - lat);
