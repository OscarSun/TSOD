...
import com.lbs.tsod.taxi1.examples.BaseLine;
import com.lbs.tsod.taxi1.model.TFragment;

import java.util.ArrayList;
import java.util.List;

public class DensityCaculateBaseLine {

    public List<TFragment> fragments;

    public DensityCaculateBaseLine() {
        fragments = new ArrayList<TFragment>();
    }

    public DensityCaculateBaseLine(List<TFragment> fragments) {
    ...
    public List<TFragment> getDensity() {
        ...
        for (int i = 0; i < fragments.size(); i++) {
            for (int j = 0; j < fragments.size(); j++) {
                ...
                list_dis.add(fragments.get(i).getDistBaseline(fragments.get(j), BaseLine.wV, BaseLine.wH, BaseLine.wX));
        ...
        for (int i = 0; i < fragments.size(); i++) {
            ...
            for (int j = 0; j < fragments.size(); j++) {
                if (fragments.get(i).getObjectId() == fragments.get(j).getObjectId()) continue;
                if (fragments.get(i).getDistBaseline(fragments.get(j), BaseLine.wV, BaseLine.wH, BaseLine.wX) < TH_DILTA) {
                    density += 1.0D;
                }
            }
            if (density == 0) {
