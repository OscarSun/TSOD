...
public class InputDirection {
    public final static String PATH = "C:\\Users\\toyking\\Desktop\\实验数据\\";
    ...
    public static void main(String args[]) throws Exception {
            ...
            for (TWay way : ways_all) {
                {
                    ...
                    for (int i = way.nodes.size() - 1; i >= 1; i--) {
                        Coordinate c = new Coordinate(), c1 = way.nodes.get(i), c2 = way.nodes.get(i - 1);
                        double len_sum = c1.distance(c2);
                        for (double len = 0; len < len_sum; len += 3e-4 + Math.random() * 1e-3) {
                            c.x = c1.x + (c2.x - c1.x) * len / len_sum + (Math.random() - 0.5) * 1e-4;
                            c.y = c1.y + (c2.y - c1.y) * len / len_sum + (Math.random() - 0.5) * 1e-4;
        ...
        // 写入文件
        {
            FileWriter fw = new FileWriter(PATH + "outlier_trajectories_direction.txt");
            fw.write("id,lng,lat,speed\n");
            for (Trajectory traj : trajectories) {
                for (TPoint p : traj.points) {
                    fw.write(p.objectID + "," + p.x + "," + p.y + "," + p.speed + "\n");
                }
            }
            fw.close();
    ...
    public static List<Trajectory> getTaxi() throws Exception {
        ...
        {
            BufferedReader br = new BufferedReader(new FileReader(PATH + "outlier_trajectories_direction.txt"));
