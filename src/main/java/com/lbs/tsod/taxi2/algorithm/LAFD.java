    ...
    public List<TFragment> getFragmentsLdd() {
        ...
        for (int i = 0; i < fragments.size(); i++) {
            ...
            for (int j = 0; j < fragments.size(); j++) {
                ...
                double dist = fragments.get(i).getDist(fragments.get(j)) * fragments.get(i).adj;

                //System.out.println("dist:" + dist+" "+fragments.get(i).getLength()+" "+fragments.get(j).getLength());
                if (dist < TH_DIST) {
                    diff += fragments.get(i).getDiff(fragments.get(j));
                    nd += 1.0;
                }
