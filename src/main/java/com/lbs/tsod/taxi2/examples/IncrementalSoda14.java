...
import com.lbs.tsod.taxi2.file.InputSoda14;
...
public class IncrementalSoda14 {
    ...
    public static void main(String args[]) throws Exception {
        ...
        final JXMapViewer mapViewer = new JXMapViewer();
        {
            ...
            JFrame frame = new JFrame("增量的轨迹异常检测SODA14日");
        ...
        List<Trajectory> all_trajs = InputSoda14.getTaxi();
        /*{
        ...
        FileWriter fw = new FileWriter("C:\\Users\\toyking\\Desktop\\Map\\Output\\outlier14.txt");
        Map<Integer, Double> object_eaf = new HashMap<Integer, Double>();
        long time = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2015-04-14 08:00:00").getTime();
        for (int t = 5; t <= 30; t += 5) {

            // 步骤1 输入轨迹
            Date begin = new Date(time + (t - 5) * 60 * 1000);
            Date end = new Date(time + t * 60 * 1000);
            List<Trajectory> input_trajs = InputSoda14.getTaxi(begin, end);
    ...
    public static void writeFile(FileWriter fw, int type, Trajectory traj) throws Exception {
        if (traj.points.size() == 132) {
            WGSTOGCJ02 wgstogcj02 = new WGSTOGCJ02();
            fw.write(type + "");
            for (int i = 0; i < traj.points.size(); i++) {
                if (i >= 50 && i <= 110) continue;
